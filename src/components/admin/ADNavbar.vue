...
const handleSearchResult = () => schedules.value.filter((item) => item.toLowerCase().includes(searchTxt.value.toLowerCase()));

const handleSide = () => {
    show.value = !show.value;
    emit('toggleSidebar', show.value);
}

const handleSmallSide = () => {
    show.value = true;
    full.value = true;
    emit('toggleSidebar', show.value);
}
...
<template>
        ...
        <div class="w-8/12 lg:w-full flex items-center justify-between"
            ...
            <div class="w-5/12 md:w-3/12 flex justify-between items-center">
                <div class="flex gap-2 sm:gap-3 md:gap-4">
                    <button @click="router.push({ name: 'AD-Shop', params: { shopName, sId } })"
    ...
    <div v-if="sm" class="w-full p-2 bg-white">
        <label for="sdashboardSearch" class="relative">
            <i
                class="absolute top-[5%] left-[3%] fa-sharp fa-light fa-magnifying-glass font-bold text-gray-500 text-sm"></i>
            <input type="text" id="sdashboardSearch"
            ...
            <ul v-if="searchTxt" class="absolute bg-white w-full z-50 rounded-b">
                <li v-for="(item, index) in handleSearchResult()" :key="index"
                    class="hover:bg-[#2f2f2f] hover:text-white px-10 py-2 cursor-pointer">
                    {{ item }}
                </li>
            </ul>
        </label>
