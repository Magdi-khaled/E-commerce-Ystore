...
import { handlePrice } from '@/composables/useUtils';
...
const deliveryFees = ref(15.5);
const subTotal = ref(0);
const discount = ref(0);
const currentProgress = ref(JSON.parse(localStorage.getItem('currentProgress')));
const fName = ref('');
const lName = ref('');
const phone = ref('');
const email = ref('');
...
const randomCode = ref(0);
const author = ref('');
const cvv = ref('');
const bank = ref('');
const expiry = ref('');
const verfiyCode = ref('');
const verfiyError = ref(false);
// local_db_data
const countries = reactive(data.countries);
const states = reactive(data.states);
...
const sendEmail = async () => {
    ...
    randomCode.value = Math.floor(100000 + Math.random() * 900000);
    try {
...
const initData = () => {
    cartItems.value.forEach(item => {
        subTotal.value += handlePrice(item.price, item.sale) * item.orderQuantity;
        discount.value += (item.sale / 100) * item.price * item.orderQuantity;
    });
    currentProgress.value = 0;
    localStorage.setItem('currentProgress', currentProgress.value)
};
const goApplication = () => {
...
const confirmPayment = () => {
    console.log('verfiyCode', verfiyCode.value);
    console.log('randomCode', randomCode.value);

    if (randomCode.value == verfiyCode.value) {
        success.value = true;
        setTimeout(() => { success.value = false }, 3000);
        verfiyError.value = false;
        success.value = true;
        setTimeout(() => {
            success = false;
